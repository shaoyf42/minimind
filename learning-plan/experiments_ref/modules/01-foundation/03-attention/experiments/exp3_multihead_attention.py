...
class SimplifiedAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 重塑 QKV 形状: [batch_size, seq_len, num_heads, head_dim]
        xq = xq.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1, 2)  # [batch_size, num_heads, seq_len, head_dim]
        xk = xk.view(batch_size, seq_len, self.num_kv_heads, self.head_dim).transpose(1, 2)  # [batch_size, num_kv_heads, seq_len, head_dim]
        xv = xv.view(batch_size, seq_len, self.num_kv_heads, self.head_dim).transpose(1, 2)  # [batch_size, num_kv_heads, seq_len, head_dim]

        # GQA: 重复 KV 头以匹配 Q 头数量
        if self.n_rep > 1:
            xk = xk.repeat_interleave(self.n_rep, dim=1)  # [batch_size, num_heads, seq_len, head_dim]
            xv = xv.repeat_interleave(self.n_rep, dim=1)  # [batch_size, num_heads, seq_len, head_dim]
        ...
        scores = scores + causal_mask.unsqueeze(0).unsqueeze(0)  # 扩展到 [1, 1, seq_len, seq_len] 以匹配分数形状
        ...
        print(f"权重之和（每行应该=1）: {attn_weights[0, 0].sum(dim=-1)[0].item():.4f}")  # 修正：计算单行的和

        # 步骤 5: 加权求和
        # 每个头的输出 = 注意力权重 @ V
        output = attn_weights @ xv  # [batch_size, num_heads, seq_len, head_dim]
        print(f"\n加权后输出: {output.shape}")

        # 步骤 6: 合并多头 + 输出投影
        output = output.transpose(1, 2).reshape(batch_size, seq_len, self.num_heads * self.head_dim)  # [batch_size, seq_len, hidden_size]
